...
-(void)initialNav{
    ...
    UIBarButtonItem *rightItem = [[UIBarButtonItem alloc]initWithBarButtonSystemItem:UIBarButtonSystemItemRefresh target:self action:@selector(resetStyle)];
...
-(void)showNot:(NSInteger)num{
    BOOL isShowing = [CRToastManager isShowingNotification];
    if (isShowing == YES){
        return;
    }
    NSString *contentString = [NSString stringWithFormat:@"随机生成行数:%ld行",num];
    NSDictionary *options = @{
                              kCRToastTextKey : contentString,
